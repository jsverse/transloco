...
- `fallbackLang`: Sets the default language to use as a fallback. See the [`TranslocoFallbackStrategy`](#transloco-fallback-strategy) section if you need to customize it.
...
```
### Using the Pipe
We suggest avoiding using a pipe, as it's not [`pure`](https://angular.io/guide/pipes#impure-pipes), and it creates a new subscription each time it's used.
...
export class AppComponent {
  ...
  ngOnInit() {
    ...
    this.service.translate('hello', { }, 'en');
  }
}
```
...
- `setActiveLang` - Sets the current active language. Calling it with `{load: true}` tells Transloco to perform a call to `loader.load()` to fetch the language
```ts
service.setActiveLang(lang);
service.setActiveLang(lang, { load: true });
```
...
service.setTranslation(lang, { ... }, { merge: false } );
```
...
).subscribe(payload => payload.lang);
...
@Component({
  ...
  templateUrl: './my-comp.component.html',
  providers: [{ 
    provide: TRANSLOCO_LOADING_TEMPLATE, useValue: '<p>loading...</p>' 
  }]
})
export class MyComponent {
}
...
```
The `handle` method is called with the failed language, and should return an array containing the next languages to load, in order of preference.
...
export function preloadUser(userService: UserService, transloco: TranslocoService) {
  return function() {
    return userService.getUser().then(({ lang }) => 
      transloco.setActiveLang(lang, { load: true }));
  };
}
