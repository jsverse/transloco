...
Next, we need to add the library's path into your [global config](../getting-started/installation#transloco-global-config) as following (we can also pass `npm` package):
