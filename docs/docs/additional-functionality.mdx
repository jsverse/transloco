...
You don't have to inject the service each time you need to translate a key or translate an object. Transloco has two exported `translate()` and `translateObject()` functions:

```ts
import { translate, translateObject } from '@ngneat/transloco';
