...
## DefaultTranspiler
The default transpiler can be configured with a custom interpolation regular expression or start and end markings to match message parameters.

When passing in a regular expression, the `DefaultTranspiler` expects one capturing group and the global flag to be enabled.

To configure the `DefaultTranspiler` you must provide a Transloco config with the interpolation property set. For example:
