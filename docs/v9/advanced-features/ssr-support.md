...
When employing Angular SSR, we need to change our loader base path to be absolute instead of relative, in order for it to work. Run `ng add @jsverse/transloco@next` and choose the SSR option. This will make sure to update the loader to use an absolute path.
