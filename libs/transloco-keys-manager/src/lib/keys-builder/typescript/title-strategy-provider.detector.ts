...
import { resolveFileList } from '../utils/extract-keys';

const translocoRouterImport = /@jsverse\/transloco\/router/;
 ...
 * AST-based, resolved through the import binding from
 * `@jsverse/transloco/router` — a named import (optionally aliased) or a
 * namespace import — and only counts as "used" if that binding is actually
 * the callee of a `CallExpression`. This means:
 ...
 */
export function detectTitleStrategyProvider(config: Config): boolean {
