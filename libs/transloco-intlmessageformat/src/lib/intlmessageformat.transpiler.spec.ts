...
import { IntlMessageFormatConfig } from './intlmessageformat.config';
...
describe('IntlMessageFormatTranspiler', () => {
    ...
    // it('should use passed-in formatters', () => { // todo: unsure if a custom formatter feat is provided in intmessageformat
    ...
    //     };
    //
    //     const parser = new IntlMessageFormatTranspiler({
    //         opts: {
    ...
    it('should switch locale in runtime', () => {
        ...
        // expect(() => parser.transpile(polishKey, params, {}, 'key')).toThrowError(); // todo: parser itself disregard language
        parser.onLangChanged('pl');
        expect(parser.transpile(polishKey, params, {}, 'key')).toBe('2 things');
    });
});

function assertParser(config: IntlMessageFormatConfig) {
    const parser = new IntlMessageFormatTranspiler(config);
