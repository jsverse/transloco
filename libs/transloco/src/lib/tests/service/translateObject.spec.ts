...
describe('translateObject', () => {
  ...
  describe('The key is an object', () => {
    it('should return an array', fakeAsync(() => {
      ...
      expect(service.translateObject(keyParamsMap)).toEqual(['b english', 'c english']);
    }));

    it('should support translating an object', fakeAsync(() => {
      loadLang(service);
      const keyParamsMap = { a: {} };
      expect(service.translateObject(keyParamsMap)).toEqual([{ b: { c: 'a.b.c {{fromList}} english' } }]);
    }));

    it('should support translation reuse', fakeAsync(() => {
      loadLang(service);
      const keyParamsMap = { a: { 'b.c': {} } };
      expect(service.translateObject(keyParamsMap)).toEqual([{ b: { c: 'a.b.c from list english' } }]);
    }));

    it('should should support params', fakeAsync(() => {
      loadLang(service);
      const keyParamsMap = { a: { 'b.c': { fromList: 'bla' } }, alert: { value: 'lang' } };
      expect(service.translateObject(keyParamsMap)).toEqual([{ b: { c: 'a.b.c bla english' } }, 'alert lang english']);
    }));
