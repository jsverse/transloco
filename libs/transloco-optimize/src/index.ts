...
import {getTranslationFiles, optimizeFiles, getTranslationsFolder} from './lib/transloco-optimize';
...
getTranslationFiles(dist)
  .then((filesPaths) => {
    if (filesPaths.length === 0) {
      return Promise.reject(`Transloco Optimize: No Translation path found under: ${getTranslationsFolder(dist)}`);
    }
