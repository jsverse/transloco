...
- Adding `provideGlobalTranslateFn()` to root providers wherever the standalone `translate()`/`translateObject()` functions are imported from `@jsverse/transloco` (detection is import-anchored, so a locally defined `translate` helper isn't mistaken for it). **Do not add this provider in SSR or multi-instance/micro-frontend apps** — inject `TranslocoService` directly there instead.
