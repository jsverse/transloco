...
if (pkgName === 'transloco-schematics') {
    console.warn(`⚠️ Notice that you are building the schematics library which is actually a part of the transloco lib`);
}

let cmd = `nx build ${pkgName}`;
if (pkgName === 'transloco') {
    cmd = [cmd, 'nx build transloco-schematics', 'rimraf dist/libs/transloco/schematics/package.json'].join('&&');
}

execSync(cmd, {stdio: 'inherit'});

function resolveSubDirs(path) {
    return readdirSync(nodePath.resolve(process.cwd(), path), { withFileTypes: true })
